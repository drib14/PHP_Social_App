...
?>
<!doctype html><html><head><meta charset="utf-8"><title>Forgot Password | Socialize</title><link rel="stylesheet" href="assets/style.css"></head><body><div class="auth-layout"><aside class="hero"><div class="hero-content"><h2 class="brand">Socialize</h2><p class="tag">Reconnect with friends and communities. Secure account recovery in seconds.</p></div></aside><main class="panel"><div class="container"><h1>Forgot password</h1><?php if($m=flash('error')): ?><div class="alert error"><?=htmlspecialchars($m)?></div><?php endif; ?><?php if($m=flash('success')): ?><div class="alert success"><?=htmlspecialchars($m)?></div><?php endif; ?><form method="post"><input type="hidden" name="csrf_token" value="<?=csrf_token()?>"><label>Email</label><input type="email" name="email" required><button>Send 6-digit code</button></form><div class="links"><a href="login.php">Back to login</a></div></div></main></div></body></html>
