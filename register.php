...
?>
<!doctype html><html><head><meta charset="utf-8"><title>Register | Socialize</title><link rel="stylesheet" href="assets/style.css"></head><body><div class="auth-layout"><aside class="hero"><div class="hero-content"><h2 class="brand">Socialize</h2><p class="tag">Join Socialize and share your moments instantly.</p></div></aside><main class="panel"><div class="container"><h1>Create account</h1><?php if($m=flash('error')): ?><div class="alert error"><?=htmlspecialchars($m)?></div><?php endif; ?><?php if($m=flash('success')): ?><div class="alert success"><?=htmlspecialchars($m)?></div><?php endif; ?><form method="post"><input type="hidden" name="csrf_token" value="<?=csrf_token()?>"><label>Name</label><input name="name" required><label>Email</label><input type="email" name="email" required><label>Password</label><input type="password" name="password" required><button>Register</button></form><div class="links"><a href="login.php">Already have account?</a></div></div></main></div></body></html>